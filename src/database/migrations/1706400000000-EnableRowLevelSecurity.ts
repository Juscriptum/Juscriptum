 ...
 *
 * CRITICAL: This must be deployed before production launch.
 *
 * How RLS works:
 * 1. Application sets tenant context via: SET app.current_tenant_id = 'uuid'
 * 2. PostgreSQL automatically filters rows based on policies
 * 3. Even if application code fails to filter, database enforces isolation
 *
 * Security guarantees:
 * - Users can ONLY see/modify data from their own tenant
 * - Cross-tenant access is impossible at database level
 * - Protects against application-level bugs and SQL injection
 */
export class EnableRowLevelSecurity1706400000000 implements MigrationInterface {
  name = "EnableRowLevelSecurity1706400000000";
