...
describe("JwtStrategy", () => {
  ...
  beforeEach(() => {
    ...
    strategy = new JwtStrategy(
      ...
      mockRevokedAccessTokenRepository as any,
    );
  ...
  it("returns current role and subscription plan from the database", async () => {
    ...
    await expect(strategy.validate(basePayload as any)).resolves.toEqual({
      user_id: "user-1",
      tenant_id: "tenant-1",
      role: "organization_admin",
  ...
  it("rejects missing users", async () => {
    ...
    await expect(strategy.validate(basePayload as any)).rejects.toThrow(
  ...
  it("rejects users without an organization relation", async () => {
    ...
    await expect(strategy.validate(basePayload as any)).rejects.toThrow(
      new UnauthorizedException("User organization not found"),
    );
  });
  ...
  it("rejects suspended organizations", async () => {
    ...
    await expect(strategy.validate(basePayload as any)).rejects.toThrow(
      new UnauthorizedException("User organization is not active"),
    );
  });
