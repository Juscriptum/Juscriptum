...
export class CdnService {
  ...
  private async purgeCloudflare(
    ...
    const purged = success
      ? options.urls?.length || options.tags?.length || 1
      : 0;

    this.logger.log(
      `Cloudflare purge: ${success ? "success" : "failed"}, purged: ${purged}`,
    );
  ...
  private async purgeCloudFront(
    ...
    const paths = options.urls?.map((url) =>
      ...
      switch (queryStringBehavior) {
        ...
        case "include-list":
          if (queryStringList?.length) {
            const filtered = queryStringList.reduce(
              (acc, param) => {
