...
let CalculationService = class CalculationService {
    async prepareCalculationItems(tenantId, itemsDto) {
        let subtotal = 0;
        const items = [];
   ...
   */ async create(tenantId, userId, dto) {
        ...
        const calculationNumber = await this.generateCalculationNumber(tenantId, dto);
        const { items, subtotal, discountAmount, vatAmount, totalAmount } = await this.prepareCalculationItems(tenantId, dto.items);
   ...
   */ async update(tenantId, id, userId, dto) {
        ...
        if (dto.dueDate !== undefined) {
            calculation.dueDate = dto.dueDate ? new Date(dto.dueDate) : dto.calculationDate ? new Date(dto.calculationDate) : calculation.calculationDate;
        }
   ...
   */ async getStatistics(tenantId) {
    ...
    /**
   * Generate calculation number
   */ async generateCalculationNumber(tenantId, dto) {
        const { caseId, caseSequence, clientNumber, operationSuffix } = await this.resolveNumberContext(tenantId, dto);
        const nextSequence = await this.getNextCalculationSequence(tenantId, caseId, clientNumber, caseSequence);
        return `${clientNumber}/${caseSequence}/${String(nextSequence).padStart(2, "0")}-${operationSuffix}`;
    }
    async resolveNumberContext(tenantId, dto) {
        const operationType = this.resolveOperationType(dto);
        const metadataClientId = typeof dto.metadata?.clientId === "string" ? dto.metadata.clientId.trim() : "";
        if (dto.caseId) {
            ...
            if (!caseEntity) {
                throw new _common.NotFoundException("Справу для формування номера розрахунку не знайдено");
            }
            if (metadataClientId && caseEntity.clientId !== metadataClientId) {
                throw new _common.BadRequestException("Клієнт розрахунку не відповідає вибраній справі");
            }
            ...
            if (!client) {
                throw new _common.NotFoundException("Клієнта справи для формування номера розрахунку не знайдено");
            }
        ...
        if (metadataClientId) {
            ...
            if (!client) {
                throw new _common.NotFoundException("Клієнта для формування номера розрахунку не знайдено");
            }
            return {
                caseId: null,
                caseSequence: "000",
    ...
    resolveOperationType(dto) {
        const metadataOperationType = dto.metadata?.operationType;
        if (metadataOperationType === "income" || metadataOperationType === "expense") {
            return metadataOperationType;
        }
        return dto.pricelistId ? "income" : "expense";
    }
    async getNextCalculationSequence(tenantId, caseId, clientNumber, caseSequence) {
        const query = this.calculationRepository.createQueryBuilder("calculation").withDeleted().where("calculation.tenantId = :tenantId", {
            tenantId
        });
        if (caseId) {
            query.andWhere("calculation.caseId = :caseId", {
        ...
        const maxSequence = calculations.reduce((maxValue, calculation)=>{
            const sequence = this.parseCalculationSequence(calculation.number, clientNumber, caseSequence);
    ...
    getClientNumber(metadata) {
        ...
        if (typeof rawValue !== "string" || !/^\d+$/.test(rawValue.trim())) {
            throw new _common.NotFoundException("У клієнта відсутній внутрішній номер для формування номера розрахунку");
        }
        return rawValue.trim();
    }
    getCaseSequence(caseNumber, clientNumber) {
        if (!caseNumber) {
            throw new _common.BadRequestException("У справі відсутній внутрішній номер для формування номера розрахунку");
        }
        const [caseClientNumber, sequencePart] = caseNumber.split("/");
        if (caseClientNumber !== clientNumber || !sequencePart || !/^\d+$/.test(sequencePart)) {
            throw new _common.BadRequestException("Неможливо сформувати номер розрахунку через некоректний внутрішній номер справи");
        }
        return sequencePart;
    }
    parseCalculationSequence(calculationNumber, clientNumber, caseSequence) {
        if (!calculationNumber) {
            return 0;
        }
        const [numberClient, numberCase, sequenceWithSuffix] = calculationNumber.split("/");
        if (numberClient !== clientNumber || numberCase !== caseSequence || !sequenceWithSuffix) {
            return 0;
        }
        const match = sequenceWithSuffix.match(/^(\d+)-[ПВ]$/);
        return match ? Number.parseInt(match[1], 10) : 0;
    }
    constructor(calculationRepository, calculationItemRepository, pricelistItemRepository, caseRepository, clientRepository){
...
CalculationService = _ts_decorate([
    ...
    _ts_param(3, (0, _typeorm.InjectRepository)(_Caseentity.Case)),
    _ts_param(4, (0, _typeorm.InjectRepository)(_Cliententity.Client)),
