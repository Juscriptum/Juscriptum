...
const _caseservice = require("./case.service");
...
describe("CaseService", ()=>{
    ...
    beforeEach(async ()=>{
        const transaction = jest.fn(async (callback)=>callback({
                getRepository: (entity)=>entity === _Caseentity.Case ? caseRepository : entity === _Evententity.Event ? eventRepository : clientRepository
            }));
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: _caseregistrysyncservice.CaseRegistrySyncService,
                    useValue: {
        ...
        service = module.get(_caseservice.CaseService);
        ...
        caseRegistrySyncService = module.get(_caseregistrysyncservice.CaseRegistrySyncService);
    ...
    it("should preview next case number as clientNumber/001 for a new client", async ()=>{
        ...
        expect(mockCaseQueryBuilder.where).toHaveBeenCalledWith("case.tenantId = :tenantId AND case.clientId = :clientId", {
            tenantId: mockTenantId,
    ...
    it("should generate internal case number on create and keep registry case number separately", async ()=>{
        ...
        caseRepository.create.mockImplementation((value)=>value);
        ...
        expect(caseRepository.create).toHaveBeenCalledWith(expect.objectContaining({
            caseNumber: "007/002",
            registryCaseNumber: "757/12345/23-ц"
        }));
    ...
    it("should throw when client has no internal number", async ()=>{
        ...
        await expect(service.getNextCaseNumber(mockTenantId, mockClientId)).rejects.toThrow(_common.NotFoundException);
    });
    it("should trigger registry sync after create", async ()=>{
        ...
        expect(caseRegistrySyncService.handleCaseLifecycleChange).toHaveBeenCalledWith(savedCase, mockUserId);
    });
    it("archives linked events when a case is archived", async ()=>{
        ...
        caseRepository.save.mockImplementation(async (value)=>value);
        const result = await service.changeStatus(mockTenantId, "case-archive", mockUserId, "archived");
    ...
    it("returns all case events in reverse chronological order without mixing in documents", async ()=>{
        ...
        expect(timeline.map((item)=>item.data.id)).toEqual([
            "event-new",
            "event-old"
        ]);
        expect(timeline.some((item)=>item.type === "document")).toBe(false);
    });
    it("skips the latest court_stan stage when it only repeats the synced court_dates hearing meaning", async ()=>{
        ...
        expect(timeline.some((item)=>item.type === "registry_stage")).toBe(false);
    });
});

//# sourceMappingURL=case.service.spec.js.map