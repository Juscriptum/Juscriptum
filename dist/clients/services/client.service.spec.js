...
describe("ClientService", ()=>{
    ...
    let caseRepository;
    let eventRepository;
    let clientNumberReleaseRepository;
    ...
    beforeEach(async ()=>{
        const transaction = jest.fn(async (callback)=>callback({
                getRepository: (entity)=>{
                    if (entity === _Cliententity.Client) {
                        return clientRepository;
                    }
                    if (entity === _Caseentity.Case) {
                        return caseRepository;
                    }
                    if (entity === _Evententity.Event) {
        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: (0, _typeorm.getRepositoryToken)(_ClientNumberReleaseentity.ClientNumberRelease),
                    useValue: {
                        findOne: jest.fn(),
                        create: jest.fn(),
                        save: jest.fn(),
        ...
        clientNumberReleaseRepository = module.get((0, _typeorm.getRepositoryToken)(_ClientNumberReleaseentity.ClientNumberRelease));
        ...
        clientNumberReleaseRepository.create.mockImplementation((value)=>value);
        ...
        caseRepository.update.mockResolvedValue({});
    ...
    describe("create", ()=>{
        ...
        it("should reuse released client number before incrementing", async ()=>{
            ...
            expect(clientNumberReleaseRepository.remove).toHaveBeenCalledWith(expect.objectContaining({
                clientNumber: 7
            }));
            expect(clientRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                metadata: {
        ...
        it("should create individual client with nested address metadata", async ()=>{
            ...
            const result = await service.create(mockTenantId, mockUserId, individualDto);
    ...
    describe("delete", ()=>{
        ...
        it("should release client number when requested", async ()=>{
            ...
            expect(clientNumberReleaseRepository.save).toHaveBeenCalledWith(expect.objectContaining({
                tenantId: mockTenantId,
                clientNumber: 1,
    ...
    describe("bulkImport", ()=>{
        ...
        it("should reject bulk import when basic plan quota would be exceeded", async ()=>{
            clientRepository.count.mockResolvedValue(1);
            await expect(service.bulkImport(mockTenantId, mockUserId, [
                {
                    type: "individual",
                    firstName: "Іван"
                }
