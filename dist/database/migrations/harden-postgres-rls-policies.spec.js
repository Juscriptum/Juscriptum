...
const _1706400000000EnableRowLevelSecurity = require("./1706400000000-EnableRowLevelSecurity");
const _1709900000000HardenUserIsolationAndTrustProviders = require("./1709900000000-HardenUserIsolationAndTrustProviders");
const _1710100000000HardenPostgresRlsPolicies = require("./1710100000000-HardenPostgresRlsPolicies");
...
describeIfPostgres("HardenPostgresRlsPolicies1710100000000", ()=>{
    ...
    beforeAll(async ()=>{
        ...
        await adminClient.query(`GRANT ALL PRIVILEGES ON DATABASE "${databaseName}" TO ${appRole}`);
        ...
        await new _1706400000000EnableRowLevelSecurity.EnableRowLevelSecurity1706400000000().up(queryRunner);
        await new _1709900000000HardenUserIsolationAndTrustProviders.HardenUserIsolationAndTrustProviders1709900000000().up(queryRunner);
        await new _1710100000000HardenPostgresRlsPolicies.HardenPostgresRlsPolicies1710100000000().up(queryRunner);
        await queryRunner.query(`GRANT USAGE ON SCHEMA public TO ${appRole}`);
        await queryRunner.query(`GRANT ALL ON ALL TABLES IN SCHEMA public TO ${appRole}`);
        await queryRunner.release();
        const appUrl = new URL(testUrl.toString());
        appUrl.username = appRole;
        appUrl.password = "";
        appDataSource = new _typeorm.DataSource({
            type: "postgres",
            url: appUrl.toString()
        });
    ...
    it("blocks cross-tenant and cross-user access for clients, cases, and documents", async ()=>{
        ...
        const visibleClients = await client.query(`SELECT id FROM clients ORDER BY id ASC`);
        const visibleCases = await client.query(`SELECT id FROM cases ORDER BY id ASC`);
        const visibleDocuments = await client.query(`SELECT id FROM documents ORDER BY id ASC`);
        expect(visibleClients.map((row)=>row.id)).toEqual([
            "c1111111-1111-1111-1111-111111111111",
            "c3333333-3333-3333-3333-333333333333"
        ]);
        expect(visibleCases.map((row)=>row.id)).toEqual([
            "a1111111-1111-1111-1111-111111111111",
            "a3333333-3333-3333-3333-333333333333"
        ]);
        expect(visibleDocuments.map((row)=>row.id)).toEqual([
    ...
    it("restricts related trust-provider records to the current actor or visible document", async ()=>{
        ...
        const visibleIdentities = await client.query(`SELECT id FROM user_identities ORDER BY id ASC`);
        const visibleSignatures = await client.query(`SELECT id FROM document_signatures ORDER BY id ASC`);
        expect(visibleIdentities.map((row)=>row.id)).toEqual([
            "e1111111-1111-1111-1111-111111111111"
        ]);
        expect(visibleSignatures.map((row)=>row.id)).toEqual([
    ...
    async function createBaseSchema(target) {
    ...
    async function seedData(target) {
        const runner = target.createQueryRunner();
        await runner.connect();
    ...
    async function setContext(runner, context) {
        await runner.query(`
        SELECT
    ...
    async function clearContext(runner) {
        await runner.query(`
        SELECT
...
//# sourceMappingURL=harden-postgres-rls-policies.spec.js.map